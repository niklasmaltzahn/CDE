...
p2 <- ggplot(CIF_diff2) + 
  ...
  geom_ribbon(aes(x = 1:365, 
                  ymin= Total_effect - 1.96 * sd_teW, 
                  ymax= Total_effect + 1.96 * sd_teW), alpha=0.2) + 
...
p3 <- ggplot(CIF_diff3) + 
  ...
  geom_ribbon(aes(x = 1:365, 
                  ymin= AW_component - 1.96 * sd_se1W, 
                  ymax= AW_component + 1.96 * sd_se1W), alpha=0.2) + 
  geom_hline(yintercept = 0, linetype = "dashed", size = 1) 
...
p4 <- ggplot(CIF_diff4) + 
  ...
  geom_ribbon(aes(x = 1:365, 
                  ymin= Mix_AGAW - 1.96 * sd_se2W, 
                  ymax= Mix_AGAW + 1.96 * sd_se2W), alpha=0.2) + 
  geom_hline(yintercept = 0, linetype = "dashed", size = 1)
